...
impl TTEntry {
    pub fn _key(&self) -> u64 { self.key }
    pub fn depth(&self) -> u8 { self.depth }
    pub fn score(&self) -> i32 { self.score }
    pub fn best_move(&self) -> Move { self.best_move }
...
impl TranspositionTable {
    ...
    pub fn clear(&mut self) { self.entries.iter_mut().for_each(|e| *e = None); }
    ...
    pub(crate) fn store(&mut self, key: u64, depth: u8, score: i32, best_move: Move, bound: Bound) {
        self.entries[(key as usize) & self.mask] = Some(TTEntry { key, depth, score, best_move, bound })
    }
}