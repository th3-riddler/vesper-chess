...
fn central_knight_beats_corner_knight_for_white() {
    ...
    assert!(evaluate(&central, &tables, &e, &weights, None, &EvalMode::Classical) > evaluate(&corner, &tables,  &e, &weights, None, &EvalMode::Classical), "centralized knight should score higher");
}
...
fn central_knight_beats_corner_knight_for_black() {
    ...
    assert!(evaluate(&central, &tables, &e, &weights, None, &EvalMode::Classical) > evaluate(&corner, &tables, &e, &weights, None, &EvalMode::Classical), "eval is from side-to-move's perspective — black benefits from centralizing too");
}
...
fn passed_pawn_bonus_is_symmetric_for_black() {
    ...
    assert!(evaluate(&board, &tables, &masks, &weights, None, &EvalMode::Classical) > 0);
}
...
fn passed_pawn_bonus_is_symmetric_in_magnitude() {
    ...
    assert_eq!(evaluate(&white_advanced, &tables, &masks, &weights, None, &EvalMode::Classical), evaluate(&black_advanced, &tables, &masks, &weights, None, &EvalMode::Classical));
}
